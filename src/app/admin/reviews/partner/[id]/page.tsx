    ...
    <div className="min-h-screen bg-linear-to-br from-gray-100 to-gray-200">
      <div className="sticky top-0 z-40 backdrop-blur-xl bg-white/70 border-b">
        <div className="max-w-7xl mx-auto h-16 px-4 flex items-center gap-4">
          ...
          {data?.partnerStatus === "approved" ? (
            <div className="px-4 py-2 rounded-full tex-xs font-semibold inline-flex items-center gap-2 bg-green-100 text-green-700">
              <Circle size={14} />
              Approved
            </div>
          ) : data?.partnerStatus === "rejected" ? (
            <div className="px-4 py-2 rounded-full tex-xs font-semibold inline-flex items-center gap-2 bg-red-100 text-red-700">
              <XCircle size={14} />
              Rejected
            </div>
          ) : (
            <div className="px-4 py-2 rounded-full tex-xs font-semibold inline-flex items-center gap-2 bg-yellow-100 text-yellow-700">
              <Clock size={14} />
              Pending
            </div>
          )}
      ...
      <main className="max-w-7xl mx-auto px-4 py-12 grid lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-8">
          ...
          <AnimatedCard title="Documents" icon={<FileText size={18} />}>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <DocPreview lable={"Aadhar"} url={partnerDocs?.aadharUrl} />
              <DocPreview
                lable={"Registration Certificate"}
        ...
        <div className="space-y-8">
          <AnimatedCard title={"Bank Dtails"} icon={<Landmark size={18} />}>
            <div className="flex justify-between text-sm">
          ...
          {data?.partnerStatus == "pending" && (
            ...
            >
              ...
              <div className="flex flex-col gap-4">
                <button
                  className="bg-linear-to-r from-black to-gray-800 text-white rounded-2xl font-semibold hover:opacity-90 transition py-3"
                  onClick={() => setShowApprove(true)}
                >
                  Approve
                </button>
                <button
                  className="py-3 rounded-2xl border font-semibold hover:bg-gray-100"
