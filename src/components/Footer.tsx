      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12">
          <div>
            ...
            <div className="flex gap-4 mt-6">
              {[ImInstagram, ImTwitter, ImFacebook, ImLinkedin].map(
                (Icon, i) => (
                  <motion.a key={i} whileHover={{y:-3, }} href="#" className="w-10 h-10 flex items-center justify-center rounded-full border border-white/20 hover:bg-white hover:text-black transition">
                    <Icon size={18}/>
                  </motion.a>
                ),
              )}
        ...
        <div className="border-t border-white/10 mt-3">
             <div className="max-w-7xl mx-auto px-6 py-6 flex flex-col sm:flex-row justify-between items-center text-xs text-gray-500 gap-4">
               <p>© {new Date().getFullYear()} RAAHI. All rights reserved.</p>
             </div>
        </div>
      </motion.div>
    </div>
